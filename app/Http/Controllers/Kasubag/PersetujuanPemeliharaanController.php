...
class PersetujuanPemeliharaanController extends Controller
{
    ...
    public function approve(Request $request, Pemeliharaan $pemeliharaan)
    {
        ...
        try {
            DB::transaction(function () use ($pemeliharaan, $selectedAssets, $request) {
                $lockedBatch = Pemeliharaan::where('batch_id', $pemeliharaan->batch_id)->lockForUpdate()->get();
                $newRejectedBatchId = (string) \Illuminate\Support\Str::uuid();
                ...
                foreach ($lockedBatch as $item) {
                    ...
                    if (in_array($item->id, $selectedAssets)) {
                        $item->update([
                            'status' => 'disetujui',
                            'approved_by' => auth()->id(),
                        ]);
            ...
            return redirect()->route('kasubag.persetujuan_pemeliharaan.index')->with('success', 'Pengajuan pemeliharaan berhasil diproses.');
        } catch (\Exception $e) {
