        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if ($role === 'operator')
                        <x-nav-link :href="route('operator.pengguna.index')" :active="request()->routeIs('operator.pengguna.*')">
                            {{ __('Kelola Pengguna') }}
                        </x-nav-link>
                        <x-nav-link :href="route('operator.aset.index')" :active="request()->routeIs('operator.aset.*')">
                            {{ __('Kelola Aset BMN') }}
                        </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->name }} ({{ ucfirst(str_replace('_', ' ', $role)) }})</div>
