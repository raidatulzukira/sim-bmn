    ...
    <div class="py-10 bg-sky-50 min-h-screen">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <form method="GET" action="{{ route('operator.aset.rekap') }}" class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('operator.aset.rekap') }}" class="px-5 py-2.5 bg-slate-100 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all duration-300">
                            Reset
                        </a>
                    </div>
                </form>
                            ...
                            @forelse($asets as $index => $aset)
                                <tr class="hover:bg-sky-50/50 transition-colors duration-200 group">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-left text-sm text-slate-600 font-medium">
                                        ...
                                        <!-- <div class="text-xs text-slate-500">{{ $aset->merk }} {{ $aset->tipe }} {{ $aset->nama }}</div> -->
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-left">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            @endif
                                            @if($aset->stok_menunggu_persetujuan > 0 || $aset->stok_menunggu_serah_terima > 0)
                                                <span class="px-2 py-0.5 inline-flex text-xs font-bold rounded-full bg-orange-50 text-orange-700">Diproses: {{ $aset->stok_menunggu_persetujuan + $aset->stok_menunggu_serah_terima }}</span>
                                            @endif
