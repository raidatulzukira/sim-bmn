                    ...
                    <form action="{{ route('operator.ruangan.update', $ruangan->id) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input id="lokasi" name="lokasi" type="text" class="block w-full border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors duration-200 bg-slate-50/50 focus:bg-white px-4 py-3" value="{{ old('lokasi', $ruangan->lokasi) }}" placeholder="Contoh: Gedung A" />
                                @error('lokasi')
                                    <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="lantai" name="lantai" type="text" class="block w-full border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors duration-200 bg-slate-50/50 focus:bg-white px-4 py-3" value="{{ old('lantai', $ruangan->lantai) }}" placeholder="Contoh: Lantai 1" />
                                @error('lantai')
                                    <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <textarea id="peruntukan" name="peruntukan" rows="3" class="block w-full border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors duration-200 bg-slate-50/50 focus:bg-white px-4 py-3" placeholder="Contoh: Tempat kegiatan operasional pegawai">{{ old('peruntukan', $ruangan->peruntukan) }}</textarea>
                            @error('peruntukan')
                                <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
